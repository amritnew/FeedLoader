...
class FeedViewControllerTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 0, "Expected no loading before view is loaded")
        
        sut.loadViewIfNeeded()
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected a loading request once view is loaded")
        
        sut.simulatedUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expected another loading request once user initiated load")
        
        sut.simulatedUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected third loading request once user initiated load")
    }
    ...
    func test_loadingFeedIndicator_isVisibleWhileLoadingFeed() {
        ...
        XCTAssertFalse(sut.isShowingLoadingIndicator(), "Expected no loading indicator once loading is complete sucessfully")
    
        sut.simulatedUserInitiatedReload()
        XCTAssertTrue(sut.isShowingLoadingIndicator(), "Expected loading indicator once user load")
   
        loader.completeFeedLoadingWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadingIndicator(), "Expected no loading indicator once user complete with error")
    }
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected second image URL requests until views become visible")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action for first view once first image loading completes successfully")
        
        let data = Data("Invalid image data".utf8)
        loader.completeImageLoading(with: data, at: 0)
        XCTAssertEqual(view?.isShowingRetryAction, true, "Expected retry action for second view once second image loading completes with error")
    ...
    func test_feedImageRetryAction_retriesImageLoad() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url, image0.url, image1.url], "Expected third image URL requests after first view retry action")
    }
    ...
    func test_feedImageView_preloadsImageURLWhenNearVisible() {
        ...
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [image0, image1])
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no image URL requests unitil image is near visible")
    ...
    func test_feedImageView_cancelImageURLPreloadsWhenNotNearVisibleAnyMore() {
        let image0 = makeImage(url: URL(string: "https://url-0.com")!)
        let image1 = makeImage(url: URL(string: "https://url-1.com")!)
        let (sut, loader) = makeSUT()
